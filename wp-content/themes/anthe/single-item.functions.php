...
function a_second_editor() {
  ...
  $if_empty = __('Place here an image gallery shortcode (Add Media &rarr; Create Gallery) or video-page URL starting with http://', A_DOMAIN);
  $c = Acorn::getm(MEDIA_CONTENT_METAKEY, $if_empty);
...
class CustomGalleryShortcode extends AShortcode {
  ...
  static function gallery ( $content, $attr ) {
    ...
    foreach ( $attachments as $id => $attachment ) {
      $item = "<img src='{$attachment->guid}' alt='{$attachment->post_title}'>";
      $output .= ($slider = true) ? "<li>{$item}</li>" : "{$item}";
    }

    $tag = ($type == 'list') ? 'ol' : 'ul';
    
    if (($int = intval($maxheight)) && !$type)
      $style = "style='max-height:{$int}px'";
    
    if ($output)
      $output = "<div class='border media-container'><{$tag} class='light slides' {$style}>{$output}</{$tag}></div>";
