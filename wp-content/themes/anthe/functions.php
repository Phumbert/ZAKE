...
include_once( '3rd/Mobile_Detect.php' );
...
function a_print_3rd_scripts () {
  ...
  if ( Acorn::get('ajaxed') ) wp_enqueue_script( 'googlemap' );
...
function a_body_class ($classes) {
  ...
  global $a_is_singlepage;
  $classes[] = $a_is_singlepage ? 'singlepage' : 'multipage';
...
function a_default_content( $content, $post ) {
  ...
  //   $content = __("[heading] The Post | Excerpt [/heading]\n\nPost content&hellip;");
...
function a_fitvids_embeds($html) {
  ...
  if ($video !== false) {
    return "<div class='media-container border'>{$html}</div><div class='clear'></div>\n";
  }
  return $html;
}
